...
namespace DotNet
{
    public partial class WeeklySalesReportGroupForPoonsh : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            Function_Method.isWarranty = true;
            Function_Method.isPBM = false;
            Function_Method.isPoonshReport = true;
        ...
        protected void ExecuteByGroup(Axapta DynAx, DateTime now)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < getGroupCode.Item5; i++)//count group code
                {
                    ...
                    if (count > 0)
                    {
                        lblAutoGenerated.Text = "";
                    }
                    if (!string.IsNullOrEmpty(getEmpGroup.Item6[0]) && regex.IsMatch(getEmpGroup.Item6[0]))
                    {
                        WeeklySalesReportGroup(DynAx, getGroupCode.Item6[i], now, getEmpGroup.Item7);
                        //MonthlySalesReportGroup(DynAx, getGroupCode.Item6[i]);
                        Debug.WriteLine(getEmpGroup.Item6[0]);
                        var getHODemailGroup = SalesReport_Get_Budget.get_HODemplIdLvl(DynAx, getGroupCode.Item6[i]);
        ...
        protected void WeeklySalesReportGroup(Axapta DynAx, string approvalLvl, DateTime now, string SalesMan)
        {
            ...
            if (DateTime.Now.Date == new DateTime(now.Year, now.Month, 1))
            {
                firstDayOfMonth = new DateTime(now.Year, now.Month - 1, 1).ToString("dd/MM/yyyy");
                lastDayOfMonth = new DateTime(now.Year, now.Month - 1, DateTime.DaysInMonth(now.Year, now.Month - 1)).ToString("dd/MM/yyyy");
                currentDate = lastDayOfMonth;
            }

            lblDescribes.Text = "Following describes the Sales Report for the following Sales ID ";
            lblDate.Text = "";
        ...
        protected void GetEmailInfo(string receiver, string hod2, string hod3, string hod4, string groupHOD, string Sendmsg)
        {
            ...
            MailSubject = "Summarized Sales Report for Group " + groupHOD;
            //Sendmsg = GetGridviewData(gvReportGroup);

            string MailTo = receiver;//recipient
            MailTo = "poonsh@posim.com.my";//tseec@posim.com.my
            Function_Method.SendMail("mailadmin", "Administrator", MailSubject, MailTo, hod4 + ",", Sendmsg);
            //Function_Method.SendMail("mailadmin", "Administrator", MailSubject, MailTo, hod4, Sendmsg);
