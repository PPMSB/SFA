<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SyncGatepassDotNetToAxV2.aspx.cs" Inherits="DotNetSync.SyncGatepassDotNetToAxV2" MaintainScrollPositionOnPostback="true" %>
...
<head runat="server">
    ...
    <title>Sync gatepass - DotNet to AX V2</title>
...
<body>
    <div class="col-12" style="padding:10px 35px; border-bottom:1px solid gray">
        <h1>Sync gatepass - DotNet to Ax</h1>
    </div>
    ...
    <script>
        ...
        function get_table() {
            ...
            if (currentTime >= startTime && currentTime < endTime) {
                $("#start_time").html("No sync after office hour");
            ...
            $.ajax({
                url: "SyncGatepassDotNetToAxV2.aspx/get_table",
                timeout: 60000,
                type: "POST",
                ...
                success: function (response) {
                    ...
                    //var invoice_array = invoice_list.split(',');
                    /*
                    for (var i = 0; i < invoice_list.length; i++) {
                        var invoice_ob = invoice_list[i];

                        var invoice_id = invoice_ob.invoice_id;
                        var coordinate = invoice_ob.coordinate;
                        var gps_date = invoice_ob.gps_date;
                        var gps_time = invoice_ob.gps_time;
                        var status = invoice_ob.status;
                        var status_by = invoice_ob.status_by;
                        var status_date = invoice_ob.status_date;

                        run_sync(invoice_id, coordinate, gps_date, gps_time, status, status_by, status_date);
                    }*/
                },
        ...
        function run_sync(invoice_id, coordinate, gps_date, gps_time, status, status_by, status_date) {
            console.log("run_sync() started: " + invoice_id);

            $.ajax({
                url: "SyncGatepassDotNetToAx.aspx/run_sync",
                timeout: 300000,
                type: "POST",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
        ...
        function update_sync_status(invoice_id) {
            console.log("update_sync_status() started: " + invoice_id);

            $.ajax({
                url: "SyncGatepassDotNetToAxV2.aspx/update_sync_status",
                timeout: 180000,
                type: "POST",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                ...
                success: function (response) {
                    ...
                    var original_content = $("#" + invoice + "_status").html();
                    $("#" + invoice + "_status").html(original_content + status_label);
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    console.log('AJAX Error:', textStatus, errorThrown);
