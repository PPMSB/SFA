<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SyncGatepassDotNetToHirev.aspx.cs" Inherits="DotNetSync.SyncGatepassDotNetToHirev" %>
...
<head runat="server">
    ...
    <title>Sync gatepass - DotNet to Hirev</title>
...
<body>
    <div class="col-12" style="padding:10px 35px; border-bottom:1px solid gray">
        <h1>Sync gatepass - DotNet to Hirev</h1>
    </div>
    ...
    <script>
        ...
        function get_table() {
            ...
            $.ajax({
                url: "SyncGatepassDotNetToHirev.aspx/get_table",
                timeout: 600000,
                type: "POST",
                ...
                success: function (response) {
                    ...
                    var gatepass_object_list = json_response.gatepass_object_list;
        ...
        function run_sync(invoice_id, invoice_date, gatepass, gatepass_date, account_num, account_name, transporter_code) {
            console.log("Sync: " + invoice_id);
            $.ajax({
                //url: "http://localhost:8080/0/sync_new/sync_gatepass_from_dotnet.php",
                //url: "https://edms.posim.com.my/0/sync_new/sync_gatepass_from_dotnet.php",
                url: "https://www.hi-rev.com.my/0/sync_new/sync_gatepass_from_dotnet.php",
                timeout: 600000,
                type: "POST",
                //contentType: "application/json; charset=utf-8",
                //dataType: "json",
                ...
                success: function (response) {
                    ...
                    var status = json_response.status;
                    var status_msg = json_response.status_msg;
                    var insert_status = json_response.insert_status;
                    var insert_status_msg = json_response.insert_status_msg;
                    var update_status = json_response.update_status;
                    var update_status_msg = json_response.update_status_msg;
                    var merge_status = json_response.merge_status;
                    var merge_status_msg = json_response.merge_status_msg;
                    ...
                    if (insert_status == "success") {
                        insert_label = "<span class='success_label'>INSERT</span>";
                    } else if (insert_status == "" || insert_status.toLowerCase() == "failed") {
                        insert_label = "<br/><span class='failed_label'>" + insert_status_msg + "</span><br/>";
                    }

                    if (update_status == "success") {
                        update_label = "<span class='success_label'>UPDATE</span>";
                    } else if (update_status == "" || update_status.toLowerCase() == "failed") {
                        update_label = "<br/><span class='failed_label'>" + update_status_msg + "</span><br/>";
                    }

                    if (merge_status == "success") {
                        merge_label = "<span class='success_label'>MERGE</span>";
                    } else if (merge_status == "" || merge_status.toLowerCase() == "failed") {
                        merge_label = "<br/><span class='failed_label'>" + merge_status_msg + "</span><br/>";
                    }
                    ...
                    $("#" + invoice + "_status").html(insert_label + update_label + merge_label);

                    // ensure either record has been sync to Hirev, and attempt to merge data has been executed
                    if (update_status == "success" || insert_status == "success") {
                        if (merge_status == "success") {
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    var status_label = "<br/><span class='failed_label'>" + 'AJAX Error:' + textStatus + "</span>";

                    var index_of_slash = invoice_id.indexOf('/');
                    invoice = invoice_id.slice(0, index_of_slash);

                    var original_html = $("#" + invoice + "_status").html();
                    $("#" + invoice + "_status").html(original_html + status_label);
        ...
        function update_success_sync_status(invoice_id) {
            ...
            $.ajax({
                url: "SyncGatepassDotNetToHirev.aspx/update_success_sync_status",
                timeout: 600000,
                type: "POST",
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                ...
                success: function (response) {
                    ...
                    // Access properties from the JSON object
                    var status = json_response.status;
                    var statusMsg = json_response.status_msg;

                    var update_sync_status_label = "";

                    if (status == "success") {
                        update_sync_status_label = "<span class='success_label'>UPDATE SYNC FLAG</span>";
                    } else if (status == "" && status.toLowerCase() == "failed") {
                        update_sync_status_label = "<br/><span class='failed_label'>" + statusMsg + "</span>";
                    }
