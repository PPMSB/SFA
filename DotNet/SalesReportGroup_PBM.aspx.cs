...
namespace DotNet
{
    public partial class SalesReportGroup_PBM : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            Function_Method.isWarranty = false;//bcc Keegan
            Function_Method.isPBM = true;//bcc to Sazila
            ...
            Function_Method.AddLog(" PBM Group Sales Report");
            ...
            try
            {
                ...
                for (int i = 0; i < getGroupCode.Item5; i++)//count group code
                {
                    ...
                    if (!string.IsNullOrEmpty(getEmpGroup.Item6[0]) && regex.IsMatch(getEmpGroup.Item6[0]))
                    {
                        ...
                        Debug.WriteLine(getEmpGroup.Item3 + getHODemailGroup.Item1 + getHODemailGroup.Item2 + getHODemailGroup.Item3 + getHODemailGroup.Item4 + getEmpGroup.Item7);
                        SalesReport.InsertSQL("PBM Group - " + getGroupCode.Item6[i]);
                        Function_Method.AddLog("- " + "PBM Group - " + getGroupCode.Item6[i]);

                        count++;
                    }
                }
                Function_Method.AddLog("- " + count.ToString() + " PBM salesman group email sent completed.");
            }
            catch (Exception ex)
            {
        ...
        protected void WeeklySalesReportGroup(Axapta DynAx, string approvalLvl, DateTime now)
        {
            ...
            try
            {
                ...
                dt.Rows.Add(row);
                gvReportGroupPBM.DataSource = dt;
                gvReportGroupPBM.DataBind();

                int row_count = gvReportGroupPBM.Rows.Count;
                string[] result; string[] delim = { "<br><br>" };
                for (int i = 0; i < row_count; i++)
                {
                    string percentage = gvReportGroupPBM.Rows[i].Cells[4].Text;
                    Debug.WriteLine(percentage);
                    result = percentage.Split(delim, StringSplitOptions.None);
                    if (Convert.ToDouble(result[0]) < 50)
                    {
                        gvReportGroupPBM.Rows[i].Cells[4].Text = "<span style='color:red'>" + result[0] + "</span><br><br>" + result[1];

                        if (Convert.ToDouble(result[1]) < 50)
                        {
                            gvReportGroupPBM.Rows[i].Cells[4].Text = "<span style='color:red'>" + result[0] + "<br><br><span style='color:red'>" + result[1] + "</span>";
                        }
                    }//change textcolor to red when percentage is not below 50

                    else if (Convert.ToDouble(result[1]) < 50)
                    {
                        gvReportGroupPBM.Rows[i].Cells[4].Text = result[0] + "<br><br><span style='color:red'>" + result[1] + "</span>";
                    }
                }
            }
            catch (Exception)
        ...
        protected void GetEmailInfo(string receiver, string hod2, string hod3, string hod4, string groupHOD)
        {
            ...
            Sendmsg = GetGridviewData(gvReportGroupPBM);
