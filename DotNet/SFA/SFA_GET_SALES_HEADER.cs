...
namespace DotNet
{
    public class SFA_GET_SALES_HEADER
    {
        ...
        */
        ...
        public static Tuple<string, int, int> get_SuffixCode(Axapta DynAx, string temp_CustomerClass)
        {
            string SuffixCode = ""; int salesmanGrace = 0;
            int hodGrace = 0;

            int MSBCustomerClass = DynAx.GetTableId("MSBCustomerClass");
            int CustomerClass = DynAx.GetFieldId(MSBCustomerClass, "CustomerClass");

            AxaptaObject axQuery2 = DynAx.CreateAxaptaObject("Query");
            AxaptaObject axQueryDataSource2 = (AxaptaObject)axQuery2.Call("addDataSource", MSBCustomerClass);

            var qbr2 = (AxaptaObject)axQueryDataSource2.Call("addRange", CustomerClass);//CUSTOMERCLASS
            qbr2.Call("value", temp_CustomerClass);

            AxaptaObject axQueryRun2 = DynAx.CreateAxaptaObject("QueryRun", axQuery2);
            while ((bool)axQueryRun2.Call("next"))
            {
                AxaptaRecord DynRec2 = (AxaptaRecord)axQueryRun2.Call("Get", MSBCustomerClass);
                SuffixCode = DynRec2.get_Field("ClassAlpha").ToString();
                salesmanGrace = Convert.ToInt16(DynRec2.get_Field("SalesmanGrace"));
                hodGrace = Convert.ToInt16(DynRec2.get_Field("HodGrace"));

                DynRec2.Dispose();
            }
            return new Tuple<string, int, int>(SuffixCode, salesmanGrace, hodGrace);
        }

        public static string get_SuffixCodeName(Axapta DynAx, string temp_SuffixCode)
        {
            string SuffixCodeName = "";
            int MSBSalesSuffix = 30002;

            AxaptaObject axQuery7 = DynAx.CreateAxaptaObject("Query");
            AxaptaObject axQueryDataSource7 = (AxaptaObject)axQuery7.Call("addDataSource", MSBSalesSuffix);

            var qbr7 = (AxaptaObject)axQueryDataSource7.Call("addRange", 30001);//SUFFIXCODE
            qbr7.Call("value", temp_SuffixCode);

            AxaptaObject axQueryRun7 = DynAx.CreateAxaptaObject("QueryRun", axQuery7);
            if ((bool)axQueryRun7.Call("next"))
            {
                AxaptaRecord DynRec7 = (AxaptaRecord)axQueryRun7.Call("Get", MSBSalesSuffix);
                SuffixCodeName = DynRec7.get_Field("SuffixDesc").ToString();
                DynRec7.Dispose();
            }
            return SuffixCodeName;
        }

        public static Tuple<string[], string[], int, string[]> get_AltAddress(Axapta DynAx, string temp_CustomeNo)
        {
            int CustTable = 77;
            ...
            // Jerry 2025-01-15 Temporary increase the size of array
            //string[] DirPtAddrRelRecId = new string[20];
            string[] DirPtAddrRelRecId = new string[120];
            // Jerry 2025-01-15 Temporary increase the size of array - END
            int counter = 0;
            if (PartyId != "")
            {
                ...
                while ((bool)axQueryRun4.Call("next"))
                {
                    ...
                    string temp = DynRec4.get_Field("RecId").ToString();

                    DirPtAddrRelRecId[counter - 1] = temp;
                    DynRec4.Dispose();

                    //if (DirPtAddrRelRecId.Length >= 20)
                    //{
                    //    return null;
                    //}
                }
            ...
            string[] Default = new string[counter];
            for (int i = 0; i < counter; i++)
            {
                ...
                if ((bool)axQueryRun6.Call("next"))
                {
                    ...
                    //Default[i] = DynRec6.get_Field("Default").ToString();//Live table dont have this field yet 1/6/23
                    AltAddress[i] = DynRec6.get_Field("Address").ToString();

                    DynRec6.Dispose();
                }
            }
            return new Tuple<string[], string[], int, string[]>(AltAddress, AddressRecId, counter, Default);
        }
        ...
        public static Tuple<string, string, string, string, string> get_AltAddress_info(Axapta DynAx, string temp_AddressRecID)
        {
            ...
            while ((bool)axQueryRun8.Call("next"))
            {
        ...
        public static Tuple<string, string, string, string, string> get_Default_Address(Axapta DynAx)
        {
            string temp_hidden_Street = ""; string temp_hidden_ZipCode = ""; string temp_hidden_City = "";
            string temp_hidden_State = ""; string temp_hidden_Country = "";
            int Address = 1;
            ...
            var qbr9 = (AxaptaObject)axQueryDataSource8.Call("addRange", 50002);//Default
            qbr9.Call("value", "1");
